...
export const useHabitStore = defineStore('habit', {
  ...
  getters: {
    ...
    monthName: (state) => {
      const lang = state.lang === 'system'
        ? (navigator.language.startsWith('ru') ? 'ru' : 'en')
        : state.lang
      const month = new Date(state.currentYear, state.currentMonth)
        .toLocaleString(lang, { month: 'long' })
      return month.charAt(0).toUpperCase() + month.slice(1)
    },
    today: (state) => {
      const today = state.realDate.getDate()
      return state.currentMonth === state.realDate.getMonth() && state.currentYear === state.realDate.getFullYear()
        ? today
        : state.daysInMonth
    },
    overallProgress: (state) => {
      if (state.tasks.length === 0) return 0.0
      const totalProgress = state.tasks.reduce((sum, task) => sum + (parseFloat(task.progress) || 0), 0)
  ...
  actions: {
    ...
    updateTaskProgress(taskId) {
      ...
      if (task && task.subtasks.length > 0) {
        const subtaskProgresses = task.subtasks.map(subtask => {
          const completed = subtask.marks.filter((status, i) => status === true && i < this.today).length
          const excluded = subtask.marks.filter((status, i) => status === false && i < this.today).length
        ...
        task.progress = (subtaskProgresses.reduce((sum, p) => sum + (p || 0), 0) / subtaskProgresses.length).toFixed(1) || 0.0
      } else if (task) {
        task.progress = 0.0
      }
      this.saveState()
     ...
     */
    exportData() {
      const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(this.archive))
    ...
    validateArchive(archive) {
      return typeof archive === 'object' && Object.values(archive).every(month =>
        month.tasks && Array.isArray(month.tasks)
      )
    },
    /**
     * Устанавливает тему.
